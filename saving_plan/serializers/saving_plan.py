...
class SavingsPlanSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        ...
        value = value.strip()
        user = self.context.get("request").user
        existing_plans = SavingsPlan.objects.filter(
    ...
    def validate_target_amount(self, value):
        ...
        try:
            ...
            if (
                ...
                and value < self.instance.target_amount
            ):
                raise serializers.ValidationError(
                    "Cannot decrease target amount of completed plan"
                )
            return value
        except (TypeError, ValueError):
            raise serializers.ValidationError("Invalid amount format")
    ...
    def validate_current_deadline(self, value):
        ...
        if self.instance and self.instance.is_completed:
            raise serializers.ValidationError(
                "Cannot modify deadline of completed plan"
            )
        return value
    ...
    def validate(self, data):
        ...
        # Handle completion status for updates
        if self.instance:
            current_saved = self.instance.get_total_saved()
            target_amount = data.get("target_amount", self.instance.target_amount)
            percentage = (
                (current_saved / target_amount * 100) if target_amount > 0 else 0
            )

            if percentage >= 100 and not self.instance.is_completed:
                self.instance.is_completed = True
            elif percentage < 100 and self.instance.is_completed:
    ...
    def get_time_remaining(self, obj):
        ...
        return f"{days_remaining} {'day' if days_remaining == 1 else 'days'} left"
