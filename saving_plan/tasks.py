...
# from saving_plan.models import DeadlineExtension, SavingsPlan
...
#     'DEADLINE_WARNING': settings.SENDGRID_DEADLINE_WARNING_TEMPLATE_ID,
#     'BEHIND_SCHEDULE': settings.SENDGRID_BEHIND_SCHEDULE_TEMPLATE_ID,
#     'PROGRESS_REPORT': settings.SENDGRID_PROGRESS_REPORT_TEMPLATE_ID,
...
#         print(SENDGRID_TEMPLATES['PLAN_CREATED'])
#         send_mail(
#             [plan.user.email],
...
#     # Fetch all active plans with an upcoming deadline
#     plans = SavingsPlan.objects.filter(
#         is_deleted=False,
#         status="ACTIVE",
#         current_deadline__gte=today  # Only consider active plans with time left
#     ).annotate(
#         total_saved=Sum("transactions__amount")
#     ).filter(
#         total_saved__lt=models.F("target_amount")  # Plans where saving is still required
#     )
...
#             continue  # Skip if deadline has passed or goal is met

#         # Calculate required savings per period dynamically
#         if plan.frequency == "MONTHLY":
#             remaining_months = max((days_remaining + 29) // 30, 1)
#             required_per_period = remaining_amount / remaining_months
#             period_start = today.replace(day=1)

#         elif plan.frequency == "WEEKLY":
#             remaining_weeks = max((days_remaining + 6) // 7, 1)
#             required_per_period = remaining_amount / remaining_weeks
#             period_start = today - timedelta(days=today.weekday())  # Start of the week

#         else:  # DAILY
#             required_per_period = remaining_amount / max(days_remaining, 1)
#             period_start = today  # Start of today

#         # Calculate savings in the current period
...
#         # If savings are below required amount, send notification
...
#                         "message": (
#                             f"You need to save {required_per_period:.2f} "
#                             f"per {plan.frequency.lower()} to meet your goal by {plan.current_deadline}."
#                         )
#                     }
...
def check_overdue_savings_plans():
    ...
    for plan in overdue_plans:
        ...
        # Auto-extend deadline logic (Hybrid Approach)
